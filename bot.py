...
async def start(bot, cmd):
	...
	if usr_cmd == "/start":
		...
		if Config.UPDATES_CHANNEL:
			...
			try:
				...
				if user.status == "kicked":
					await bot.send_message(
						chat_id=cmd.from_user.id,
						text="Sorry Sir, You are Banned to use me. Contact my [Support Group](https://t.me/moviez_cafe_38).",
						parse_mode="markdown",
						disable_web_page_preview=True
					)
					return
				...
				await bot.send_message(
					...
					reply_markup=InlineKeyboardMarkup(
						[
							...
							[
								InlineKeyboardButton("🔄 Refresh / Try Again", url=f"https://telegram.dog/{BOT_USERNAME}?start=moviezcafe38_{file_id}")
							]
						]
					),
					parse_mode="markdown"
				...
				await bot.send_message(
					chat_id=cmd.from_user.id,
					text="Something went Wrong. Contact my [Support Group](https://t.me/moviez_cafe_38).",
					parse_mode="markdown",
					disable_web_page_preview=True
				)
				return
		try:
			...
			await send_stored_file.reply_text(f"**Here is Sharable Link of this file:** https://telegram.dog/{BOT_USERNAME}?start=moviezcafe38_{file_id}\n\n__To Retrive the Stored File, just open the link!__", disable_web_page_preview=True, quote=True)
...
async def main(bot, message):
	if message.chat.type == "private":
		...
		try:
			...
			share_link = f"https://telegram.dog/{BOT_USERNAME}?start=moviezcafe38_{file_er_id}"
			await editable.edit(
